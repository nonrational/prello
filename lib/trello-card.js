...
TrelloCard.prototype.comment = function(comment_text, callback){
    ...
    this.t.post(comment_url, { text: comment_text }, callback);
};
...
TrelloCard.prototype.delete_comment = function(comment_id, callback){
    var comment_url = util.format("/1/cards/%s/actions/%s/comments", this.id, comment_id);
    this.t.del(comment_url, callback);
};

TrelloCard.prototype.delete_comment_like = function(comment_text, callback){
    var comments_url = util.format("/1/cards/%s/actions", this.id),
        self = this;
    ...
    this.t.get(comments_url, {}, function(e,d){
        for (var i = d.length - 1; i >= 0; i--) {
            if(d[i].data.text === comment_text){
                self.delete_comment(d[i].id, callback);
            }
        }
    });
};
